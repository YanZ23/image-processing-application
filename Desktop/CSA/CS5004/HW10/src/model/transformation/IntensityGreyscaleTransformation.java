...
/**
 * The transformation that converts an image to intensity greyscale.
 */
public class IntensityGreyscaleTransformation implements ITransformation {
  ...
  /**
   * Gets the intensity value of a pixel in the given image state at the specified position.
   *
   ...
   */
  private int getImageIntensity(IImageState sourceImage, int row, int col) {
  ...
  public IImageState run(IImageState sourceImage) {
    ...
    for (int row = 0; row < sourceImage.getHeight(); row++) {
      for (int col = 0; col < sourceImage.getWidth(); col++) {
        int pixelValue = getImageIntensity(sourceImage, row, col);
