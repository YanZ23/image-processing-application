...
/**
 * The command that transforms an image into intensity greyscale.
 */
public class IntensityGreyscaleCommand implements ICommand {
  private final IImageDataBase model;

  /**
   * Instantiates a new IntensityGreyscaleCommand with the specified image database.
   *
   * @param model The image database to store and retrieve images.
   */
  public IntensityGreyscaleCommand(IImageDataBase model) {
    this.model = model;
  }
  ...
  public void run(Scanner scanner, IImageDataBase model) {
    ...
    IImageState greyedImage = intensityGreyscaleTransformation.run(sourceImage);
