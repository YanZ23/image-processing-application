...
/**
 * The command that uses filter to transform an image into intensity greyscale.
 */
public class IntensityGreyscaleFilterCommand implements ICommand {
  private final IImageDataBase model;

  /**
   * Instantiates a new IntensityGreyscaleFilterCommand with the specified image database.
   *
   * @param model The image database to store and retrieve images.
   */
  public IntensityGreyscaleFilterCommand(IImageDataBase model) {
    this.model = model;
  }
